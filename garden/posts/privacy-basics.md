...
The word "tracking" gets used loosely to describe very different technical mechanisms. Logging a visit with a cookie, recognizing you across sites with a pixel, and identifying your browser configuration with no stored data at all are three separate things. They have different threat models and different defenses.
...
When you click a link with UTM parameters, those parameters travel to the destination server in the URL. The server logs them. They tell whoever runs the site that you arrived from a newsletter, via email, as part of a spring sale campaign. The parameters survive as long as the URL survives. If you copy and share that link, anyone who clicks it also passes those tags to the destination. Nothing changes in your browser. No cookie is set by the URL itself. The logging happens server-side, on a machine you have no access to.
