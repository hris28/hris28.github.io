...
async function initGardenEntry() {
  ...
  const head = `
    <a class="back-link" href="garden.html">← Back to the garden</a>
    <p class="eyebrow">${TYPE_LABEL[e.type] || e.type}${mat ? ` · ${mat.icon} <span class="stage-${e.maturity}">${mat.label}</span>` : ""}</p>
  ...
  if (e.video) {
    body += `<video class="media-video" controls preload="metadata" src="${e.video}"></video>`;
  }
  if (e.audio) {
